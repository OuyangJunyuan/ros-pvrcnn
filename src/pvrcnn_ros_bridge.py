...
import rospy
import actionlib
from ros_pvrcnn.msg import detector3dAction, detector3dResult, box3d
...
class pvrcnn_ros:
    def __init__(self, input_dict):
        ...
        # for ROS
        self.action_server = actionlib.SimpleActionServer("excavator/lidar_perception/ros_pvrcnn_action",
                                                          detector3dAction,
    ...
    def detector(self, points):
        with torch.no_grad():
            ...
            pred_boxes = pred_dicts['pred_boxes'].detach().cpu().numpy()[indices]
            pred_labels = pred_dicts['pred_labels'].detach().cpu().numpy()[indices]
    ...
    def execute_cb(self, request):
        print('-------------------------------------')
        print(request.point_cloud.header.seq)

        header, points_msg = request.point_cloud.header, request.point_cloud
        points_raw = rnp.point_cloud2.pointcloud2_to_xyz_array(points_msg)
        points_raw = np.hstack((points_raw, np.zeros([len(points_raw), 1])))

        boxes, scores, labels = self.detector(points_raw)

        result = detector3dResult()
        result.header = header
        result.cls_list = self.cls_list
        # print(result.cls_list)
        for box, label, score in zip(boxes, labels, scores):
            box_ins = box3d()
            box_ins.id = label - 1
            box_ins.cls = cfg.CLASS_NAMES[box_ins.id]
            box_ins.score = score
            box_ins.xyz = box[0:3]
            box_ins.lwh = box[3:6]
            box_ins.heading = box[6]
            result.result.append(box_ins)
